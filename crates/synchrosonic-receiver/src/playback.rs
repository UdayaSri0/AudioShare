...
use synchrosonic_core::{ReceiverError, ReceiverStreamConfig};
...
impl PlaybackEngine for LinuxPlaybackEngine {
    ...
    fn start_stream(
        ...
        tracing::info!(
            ...
            "starting receiver playback stream"
        );
        ...
        let mut child = command.spawn().map_err(|source| match source.kind() {
            std::io::ErrorKind::NotFound => {
                ReceiverError::PlaybackUnavailable(self.pw_play_bin.display().to_string())
            }
            _ => ReceiverError::PlaybackStart {
                command: self.pw_play_bin.display().to_string(),
                source,
            },
        })?;

        let stdin = child.stdin.take().ok_or_else(|| ReceiverError::PlaybackIo {
                ...
                for line in reader.lines() {
                    match line {
                        Ok(line) if !line.trim().is_empty() => {
                            tracing::debug!(
                                target: "synchrosonic_receiver::pipewire",
                                message = %line,
                                "pw-play stderr"
                            );
                        }
...
impl PlaybackSink for PipeWirePlaybackSink {
    fn write(&mut self, payload: &[u8]) -> Result<(), ReceiverError> {
        let stdin = self.stdin.as_mut().ok_or_else(|| ReceiverError::PlaybackIo {
        ...
        stdin
            .write_all(payload)
            .map_err(|source| ReceiverError::PlaybackIo {
    ...
    fn stop(&mut self) -> Result<(), ReceiverError> {
        ...
        if let Some(mut child) = self.child.take() {
            match child.try_wait() {
                Ok(Some(_status)) => {}
                Ok(None) => {
                    child.kill().map_err(|source| ReceiverError::PlaybackIo {
                        context: "stopping pw-play".to_string(),
                        source,
                    })?;
                    child.wait().map_err(|source| ReceiverError::PlaybackIo {
                        context: "waiting for pw-play to stop".to_string(),
                        source,
                    })?;
                }
                Err(source) => {
                    return Err(ReceiverError::PlaybackIo {
...
fn validate_playback_request(request: &PlaybackStartRequest) -> Result<(), ReceiverError> {
